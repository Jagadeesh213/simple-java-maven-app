...
//        sh 'aws s3 cp webapp/target/webapp.war s3://demo-test198'
    ...
    stage('Deploy to tomcat') {
      steps {
      sh 'sudo scp -i $tomcat_key -o "StrictHostKeyChecking=no" /var/lib/jenkins/workspace/maven build/webapp/target/webapp.war  ubuntu@15.206.170.20:/opt/tomcat/webapps'
